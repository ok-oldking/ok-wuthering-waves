    ...
    def do_perform(self):
        ...
        if self.judge_forte() == 2 and not self.task.find_one('lupa_wolf_icon2', threshold=0.85):
            ...
            if self.flying():
                ...
                self.heavy_attack()
                self.task.wait_until(lambda: not self.is_forte_full(), post_action=self.click_with_interval,
                                     time_out=1.4)
            if not self.is_forte_full():
                self.wolf = True
            if in_outro:
                self.task.wait_until(lambda: self.task.find_one('lupa_wolf_icon2', threshold=0.85), post_action=None,
                                     time_out=0.5)
                self.res_wolf()
            else:
                return self.switch_next_char()

...
class BaseChar:
    ...
    def current_resonance_circle(self, circle_color=text_white_color, inner_radius_ratio=0.9):
        """获取当前共鸣技能圆环UI颜色像素百分比。"""
        ...
        return self.calculate_color_percentage_in_circle(
            circle_color, box, inner_radius_ratio=inner_radius_ratio
        )

    def current_liberation_circle(self, circle_color=text_white_color, inner_radius_ratio=0.9):
        """获取当前共鸣解放圆环UI颜色像素百分比。"""
        box = self.task.box_of_screen_scaled(
    ...
    def calculate_color_percentage_in_circle(
        self, target_color, box, inner_radius_ratio=0.9, outer_radius_ratio=1.0
    ):
        """计算指定圆环区域内目标颜色的像素百分比
        Args:

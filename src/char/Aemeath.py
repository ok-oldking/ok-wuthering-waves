...
class Aemeath(BaseChar):
    ...
    def do_perform(self):
        ...
        if self.has_intro:
            ...
            # 变奏入场：普攻起手后尽快放R1（变奏+R1=2星），R1后才可施放强化技能
            self.continues_normal_attack(0.8)
            outro = self.check_outro()
            if outro in {'char_linnai', 'char_lupa'}:
                self.intro_time = 14
            if outro in {'chang_changli', 'char_changli2'}:
                self.intro_time = 10
    ...
    def heavy_wait_highlight_down(self):
        ...
        try:
            return self.task.wait_until(lambda: not self.has_long_action(), time_out=1.2)
        finally:
            self.task.mouse_up()
            self.sleep(0.01)
    ...
    def get_switch_priority(self, current_char=None, has_intro=False, target_low_con=False):
        if self.should_wait_for_lib2():
            # Let Linnai consume Mornye's outro instead of competing with Aemeath's pending R2.

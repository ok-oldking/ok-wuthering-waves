...
class Zhezhi(BaseChar):
    ...
    def con_lock(self):
        return self.char_carlotta.get_ready() or self.get_current_con()<0.6 or (self.is_con_full() and not self.char_carlotta.get_ready())
    ...
    def do_perform_interlock(self):       
        ...
        if not self.resonance_blue() and self.judge_forte() < 3 and not (self.is_con_full() and self.char_carlotta.get_ready()):            

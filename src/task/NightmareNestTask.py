...
class NightmareNestTask(WWOneTimeTask, BaseCombatTask):
    ...
    def combat_nest(self, nest):
        ...
        try:
            ...
            # 死亡恢复后直接跳过当前 nest（不重试），原因：
            # 噩梦巢穴死亡意味着角色练度不足或配队问题，重试大概率再次阵亡；
            # 跳过当前 nest 后继续下一个巢穴，避免卡死在同一个 boss 上无限循环。
            self.log_info('combat_nest: death recovery, skip nest cleanup and move to next nest')
            try:

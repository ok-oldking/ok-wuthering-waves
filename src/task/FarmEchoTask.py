...
class FarmEchoTask(BaseCombatTask):
    ...
    def run(self):
        ...
        if self.config.get('Teleport'):
            book = self.find_one('gray_book_button')
            if not book:
            ...
            self.send_key('f2')
            self.wait_click_feature('gray_book_forgery', raise_if_not_found=True)
            self.wait_click_feature('gray_book_weekly_boss', raise_if_not_found=True)
            self.sleep(1)
            self.click_relative(self.crownless_pos[0], self.crownless_pos[1])
            self.wait_click_feature('gray_teleport', raise_if_not_found=True)
        ...
        while count < self.config.get("Repeat Farm Count", 0):
            ...
            self.walk_until_f()
            self.wait_click_feature('gray_crownless_battle', raise_if_not_found=True)
